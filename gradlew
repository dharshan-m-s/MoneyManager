...
if [ ! -x "$GRADLE_BIN" ]; then
  mkdir -p "$CACHE_DIR"
  if [ ! -f "$DIST_ZIP" ]; then
    if command -v curl >/dev/null 2>&1; then curl -fL --retry 3 --connect-timeout 20 -o "$DIST_ZIP" "$DIST_URL";
    elif command -v wget >/dev/null 2>&1; then wget -O "$DIST_ZIP" "$DIST_URL";
    else echo "curl or wget is required to bootstrap Gradle $GRADLE_VERSION" >&2; exit 1; fi
  fi
