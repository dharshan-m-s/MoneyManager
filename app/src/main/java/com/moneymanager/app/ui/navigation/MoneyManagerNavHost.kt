...
import com.moneymanager.app.ui.income.IncomeSummaryScreen
...
import com.moneymanager.app.ui.categories.EditCategoryScreen
    ...
    NavHost(
        ...
        composable(Destination.Dashboard.route) {
            DashboardScreen(
                ...
                onSeeAllIncomeAreas = { navController.navigate(Destination.IncomeSummary.route) },
        ...
        composable(Destination.SpendSummary.route) {
            SpendSummaryScreen(onBack = { safeBack() }, onCategoryClick = { id -> navController.navigate(Destination.CategoryTransactions.route(id, TxnSubType.EXPENSE, true)) })
        }
        composable(Destination.IncomeSummary.route) {
            IncomeSummaryScreen(
                onBack = { safeBack() },
                onCategoryClick = { id, subType, included ->
                    navController.navigate(Destination.CategoryTransactions.route(id, subType, included))
                },
                onTransactionClick = { tx -> navController.navigate(Destination.TransactionDetail.route(tx)) }
            )
        }
        composable(Destination.Categories.route) {
            CategoryManagerScreen(
                ...
                onEditCategory = { id -> navController.navigate(Destination.EditCategory.route(id)) }
            )
        }
        ...
        composable(
            route = Destination.EditCategory.route,
        ...
        composable(
            route = Destination.CategoryTransactions.route,
            arguments = listOf(
                navArgument("categoryId") { type = NavType.LongType },
                navArgument("subType") { type = NavType.StringType; defaultValue = TxnSubType.EXPENSE.name },
                navArgument("included") { type = NavType.BoolType; defaultValue = true }
            )
        ) { entry ->
            val id = entry.arguments?.getLong("categoryId") ?: -1L
            val subType = entry.arguments?.getString("subType")
                ?.let { runCatching { TxnSubType.valueOf(it) }.getOrNull() }
                ?: TxnSubType.EXPENSE
            val included = entry.arguments?.getBoolean("included") ?: true
        ...
        composable(Destination.AppUpdates.route) {
            com.moneymanager.app.ui.updates.UpdateCenterScreen(onBack = { safeBack() })
        }
