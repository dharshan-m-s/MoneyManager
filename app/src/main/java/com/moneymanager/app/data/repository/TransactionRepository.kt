...
import com.moneymanager.app.domain.model.TransactionDirectionColumns
import com.moneymanager.app.domain.model.TransactionDirectionResolver
...
data class NewTransactionInput(
    ...
    /** Staged private receipt path, finalized only after the transaction id exists. */
...
class TransactionRepository @Inject constructor(
    ...
    suspend fun addTransaction(input: NewTransactionInput): Long {
        return database.withTransaction {
            ...
            val direction = TransactionDirectionResolver.resolve(input.amount.minorUnits, input.isIncome, input.paymentType)
            val fingerprint = fingerprint(
                input.occurredAtEpochMillis, input.accountId, direction.creditMinorUnits, direction.debitMinorUnits,
            ...
            val entity = TransactionEntity(
                ...
                rawDateString = "",
                txnType = direction.txnType,
                txnSubType = direction.txnSubType,
                txnKind = direction.txnKind,
                ...
                creditMinorUnits = direction.creditMinorUnits,
                debitMinorUnits = direction.debitMinorUnits,
            ...
            val finalReceipt = receiptStorage.finalizeForTransaction(id, input.receiptTempPath)
            if (finalReceipt != null) {
                val saved = transactionDao.findById(id)
                if (saved != null) transactionDao.update(saved.copy(receiptPath = finalReceipt, updatedAtEpochMillis = System.currentTimeMillis()))
            }
     ...
     *  canonical accounting engine - the recompute deterministically reverses the old effect of
     *  the row and applies the new one (Spend → Income flips the ledger movement exactly once).
     *
     *  @param isIncome when false the transaction is recorded as a Spend (debit). Passing the
     *  desired income/expense direction here is the ONLY source of truth; the existing row's
     *  sign does not silently win.
     */
    suspend fun updateEditableTransaction(
        ...
        database.withTransaction {
            ...
            previousReceipt = existing.receiptPath
            val isTransfer = existing.txnSubType == TxnSubType.TRANSFER_IN || existing.txnSubType == TxnSubType.TRANSFER_OUT
            // Transfers keep their two-sided columns verbatim; everything else is re-derived
            // from the user's chosen income/spend direction (the ONLY source of truth).
            val dir = if (isTransfer) TransactionDirectionColumns(
                creditMinorUnits = existing.creditMinorUnits,
                debitMinorUnits = existing.debitMinorUnits,
                txnType = existing.txnType,
                txnSubType = existing.txnSubType,
                txnKind = existing.txnKind
            ) else TransactionDirectionResolver.resolve(amount.minorUnits, isIncome, existing.paymentType)
            val fingerprint = fingerprint(existing.occurredAtEpochMillis, existing.accountId, dir.creditMinorUnits, dir.debitMinorUnits, merchantReceiverSender, existing.rawCategoryName)
            transactionDao.update(existing.copy(
                creditMinorUnits = dir.creditMinorUnits, debitMinorUnits = dir.debitMinorUnits,
                txnType = dir.txnType, txnSubType = dir.txnSubType, txnKind = dir.txnKind,
                ...
                receiptPath = if (removeReceipt) null else existing.receiptPath,
                updatedAtEpochMillis = System.currentTimeMillis()
            ))
            accountingService.recalculateAccounts(setOf(existing.accountId))
        }.also {
            if (removeReceipt) {
                previousReceipt?.let { receiptStorage.delete(it) }
            } else if (!receiptTempPath.isNullOrBlank()) {
                val finalReceipt = receiptStorage.finalizeForTransaction(transactionId, receiptTempPath)
                if (finalReceipt != null) {
                    val saved = transactionDao.findById(transactionId)
                    if (saved != null) {
                        transactionDao.update(saved.copy(receiptPath = finalReceipt, updatedAtEpochMillis = System.currentTimeMillis()))
                        previousReceipt?.takeIf { it != finalReceipt }?.let { receiptStorage.delete(it) }
                    } else {
                        receiptStorage.delete(finalReceipt)
                    }
                }
            }
            backupManager.scheduleAfterWrite()
    ...
    suspend fun attachReceipt(transactionId: Long, stagedPath: String?): Boolean {
        if (stagedPath.isNullOrBlank()) return false
        val finalReceipt = receiptStorage.finalizeForTransaction(transactionId, stagedPath) ?: return false
        val existing = transactionDao.findById(transactionId) ?: run { receiptStorage.delete(finalReceipt); return false }
        transactionDao.update(existing.copy(receiptPath = finalReceipt, updatedAtEpochMillis = System.currentTimeMillis()))
        existing.receiptPath?.let { old -> if (old != finalReceipt) receiptStorage.delete(old) }
        backupManager.scheduleAfterWrite()
        return true
    }
    ...
    suspend fun deleteTransaction(transactionId: Long) {
        database.withTransaction {
            val existing = transactionDao.findById(transactionId) ?: return@withTransaction
            if (existing.txnSubType == TxnSubType.TRANSFER_IN || existing.txnSubType == TxnSubType.TRANSFER_OUT) return@withTransaction
