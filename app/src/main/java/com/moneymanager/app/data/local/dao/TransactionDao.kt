...
interface TransactionDao {
    ...
    suspend fun searchTransactionsOnce(query: String, limit: Int = 5000): List<TransactionEntity>
    ...
    @Query(
        "SELECT categoryId, COALESCE(SUM(creditMinorUnits - debitMinorUnits), 0) as total FROM transactions " +
            "WHERE txnSubType = 'INCOME' AND includeInStatistics = 1 " +
            "AND occurredAtEpochMillis BETWEEN :startInclusive AND :endExclusive " +
            "GROUP BY categoryId ORDER BY total DESC"
    )
    fun observeIncomeCategoryTotalsInRange(startInclusive: Long, endExclusive: Long): Flow<List<CategoryTotal>>

    @Query(
        "SELECT * FROM transactions WHERE txnSubType = 'INCOME' AND includeInStatistics = 0 " +
            "AND occurredAtEpochMillis BETWEEN :startInclusive AND :endExclusive " +
            "ORDER BY occurredAtEpochMillis DESC"
    )
    fun observeExcludedIncomeInRange(startInclusive: Long, endExclusive: Long): Flow<List<TransactionEntity>>

    @Query(
        "SELECT * FROM transactions WHERE ((:categoryId IS NULL AND categoryId IS NULL) OR categoryId = :categoryId) " +
            "AND txnSubType = :txnSubType AND includeInStatistics = :included " +
            "ORDER BY occurredAtEpochMillis DESC"
    )
    fun observeByCategoryAndIncluded(categoryId: Long?, txnSubType: String, included: Boolean): Flow<List<TransactionEntity>>
