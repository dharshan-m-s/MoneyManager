...
class AccountingEngineScenariosTest {
    ...
    fun `scenario12 refund reversal returns money and is not spend`() {
        ...
        // A reversal carries negative Debit in the source. The signed debit is preserved so the
        // estimated formula (lastReported + adjustedCredit - adjustedDebit) raises the balance.
    ...
    fun `scenario16b post-checkpoint adjustments reproduce dashboard formula`() {
        ...
        // The dashboard/estimated formula equals the snapshot-anchored calculated balance:
        // 1000 (lastReported) + 50 (adjustedCredit) - 250 (adjustedDebit) = 800.
